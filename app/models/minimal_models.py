...
class Movie(Base):
    ...
    # Details
    release_date: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)  # first_air_date for TV
    last_air_date: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)  # for TV only
    runtime: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # for movies
    number_of_seasons: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # for TV
    number_of_episodes: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # for TV
    ...
    genres: Mapped[Optional[list]] = mapped_column(JSON, default=list)  # List of genre names
    cast: Mapped[Optional[list]] = mapped_column(JSON, default=list)  # List of cast members
    director: Mapped[Optional[str]] = mapped_column(String(200), nullable=True)  # for movies
    creator: Mapped[Optional[str]] = mapped_column(String(200), nullable=True)  # for TV
    networks: Mapped[Optional[list]] = mapped_column(JSON, default=list)  # for TV
    production_companies: Mapped[Optional[list]] = mapped_column(JSON, default=list)
    
    # Streaming Info
    streaming_info: Mapped[Optional[dict]] = mapped_column(JSON, default=dict)  # Where to watch
    
    # Content
    preview_content: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # Generated by ContentAnalyzer
    human_summary: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # Editor's pick
    
    # Admin flags
    is_approved: Mapped[bool] = mapped_column(Boolean, default=True)  # Auto-approved by default
    is_trending: Mapped[bool] = mapped_column(Boolean, default=False)  # Marked as trending
    ...
    # Relationships
    similar_movies: Mapped[Optional[list]] = mapped_column(JSON, default=list)  # Store similar movie IDs
